...
int save_princess(const vector<vector<int>>& castle, const int castle_row, const int castle_col, const int cursing_time)
{
	vector<vector<bool>> visited(castle_row, vector<bool>(castle_col));
	queue<warrior> warrior_move;
	warrior_move.push({ 0,0,0 });
	visited[0][0] = true;
	int move_count = 987654321;
	while (!warrior_move.empty())
	{
		const warrior current_state = warrior_move.front();
		warrior_move.pop();
		if (current_state.row == castle_row - 1 && current_state.col == castle_col - 1)
		{
			move_count = min(move_count, current_state.time_take_to_move);
			break;
		}
		const int time_to_next_move = current_state.time_take_to_move + 1;
		if (time_to_next_move > cursing_time)
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (castle[next_row][next_col] == 2)
			{
				move_count = min(move_count, time_to_next_move + abs(castle_row - 1 - next_row + castle_col - 1 - next_col));
				continue;
			}
			warrior_move.push({ next_row , next_col, time_to_next_move });
			visited[next_row][next_col] = true;
...
int main()
{
	...
	const int answer = save_princess(castle, castle_row, castle_col, cursing_time);
	if (answer == 987654321)
	{
		cout << "Fail\n";
	}
